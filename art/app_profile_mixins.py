...
from .models import ArtAppProfile
# Model by tag mixin

class NewModelListMixin:
    '''
    Queryset and context for new list views using ArtAppProfile.
    '''
...
class TopModelListMixin:
    '''
    Queryset and context for top list views using ArtAppProfile.
    '''
    def get_queryset(self, *args, **kwargs):
        context = super().get_queryset(**kwargs)
        return self.model.objects.filter(
...
class ModelByMemberMixin:
    ...
    the view. Also sets the ArtAppProfile.
    '''
    def get_queryset(self, *args, **kwargs):
