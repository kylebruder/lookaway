...
env_var_file="/home/$USER/.profile"
...
if [ ! -f $env_var_file ]; then
    echo "'$env_var_file' does not exist. Create it now? (yes, no)"
    read;
    if [ ${REPLY} == "yes" ]; then
        touch $env_var_file
        chmod 600 $env_var_file
        if [ $? ]; then
            echo "Successfully created '$env_var_file'."
        else
            echo "Could not create the file '$env_var_file'. Check permissions or disk space."
            exit
        fi
    else
        echo "No file was created. Aborting."
        exit 
    fi
fi
    ...
    case $var in
        "Django Secret Key (autogenerated)")
            ...
            if ! egrep "^DJANGO_SECRET_KEY=" $env_var_file > /dev/null ; then
                echo "export DJANGO_SECRET_KEY='$(`dirname "$BASH_SOURCE"`/make_key.py)'" >> $env_var_file
            if [ $? ]; then
